...
object CsvBackup {
    ...
    fun parse(text: String): Map<String, List<Map<String, String>>> {
        ...
        splitCsvRows(text).forEach { line ->
            when {
                line.startsWith("#") -> {
    ...
    /**
     * Splits the whole backup text into rows, the same job as `text.split(newline)` except it
     * doesn't break a row on a newline that's inside a quoted field. [csvEscape] quotes a field
     * that contains "\n" (e.g. a multi-line pasted note) rather than stripping it, so a naive
     * whole-text split on newline would silently cut that field's row in two on restore.
     * Quote-tracking here doesn't need to distinguish an escaped "" from a real closing quote:
     * two toggles in a row cancel out, so the in/out-of-quotes state stays correct either way.
     */
